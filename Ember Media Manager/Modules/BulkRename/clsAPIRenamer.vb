...
#Region "Methods"
        ...
        Public Shared Sub RenameSingle(ByRef _tmpMovie As Model.Movie, ByVal folderPattern As String, ByVal filePattern As String, ByVal BatchMode As Boolean, ByVal toNfo As Boolean, ByVal ShowError As Boolean)
            Dim MovieFile As New FileRename

            If Not IsNothing(_tmpMovie.FileInfo) Then
                Try
                    If _tmpMovie.FileInfo.StreamDetails.Video.Count > 0 Then
                        Dim tVid As MediaInfo.Video = MediaInfo.GetBestVideo(_tmpMovie.FileInfo)
                        Dim tRes As String = MediaInfo.GetResFromDimensions(tVid)
                        MovieFile.Resolution = String.Format("{0}", If(String.IsNullOrEmpty(tRes), Languages.Unknown, tRes))
                    Else
                        MovieFile.Resolution = String.Empty
                    End If

                    If _tmpMovie.FileInfo.StreamDetails.Audio.Count > 0 Then
                        Dim tAud As MediaInfo.Audio = MediaInfo.GetBestAudio(_tmpMovie.FileInfo, False)
                        MovieFile.Audio = String.Format("{0}-{1}ch", If(String.IsNullOrEmpty(tAud.Codec), Languages.Unknown, tAud.Codec), If(String.IsNullOrEmpty(tAud.Channels), Master.eLang.GetString(283, Languages.Unknown, True), tAud.Channels))
                    Else
                        MovieFile.Audio = String.Empty
                    End If
                Catch ex As Exception
                    Master.eLog.WriteToErrorLog(ex.Message, ex.StackTrace, Languages.Error_FileInfo)
                End Try
            Else
                MovieFile.Resolution = String.Empty
                MovieFile.Audio = String.Empty
            ...
            MovieFile.OriginalTitle = _tmpMovie.OriginalTitle
            MovieFile.Year = _tmpMovie.Year
            MovieFile.IsSingle = _tmpMovie.UseFolder
            MovieFile.SortTitle = _tmpMovie.SortTitle
            MovieFile.Genre = _tmpMovie.Genre
            MovieFile.Director = _tmpMovie.Director
            MovieFile.FileSource = _tmpMovie.FileSource
            MovieFile.Country = _tmpMovie.Country
            ...
            For Each i As String In mFolders
                If _tmpMovie.MoviePath.StartsWith(i, StringComparison.OrdinalIgnoreCase) Then
                    MovieFile.BasePath = i
                    If FileUtils.isVideoTS(_tmpMovie.MoviePath) Then
                        MovieFile.Parent = Directory.GetParent(Directory.GetParent(_tmpMovie.MoviePath).FullName).Name
                        If MovieFile.BasePath = Directory.GetParent(Directory.GetParent(_tmpMovie.MoviePath).FullName).FullName Then
                            MovieFile.OldPath = String.Empty
                            MovieFile.BasePath = Directory.GetParent(MovieFile.BasePath).FullName
                        Else
                            MovieFile.OldPath = Directory.GetParent(Directory.GetParent(Directory.GetParent(_tmpMovie.MoviePath).FullName).FullName).FullName.Replace(i, String.Empty)
                        End If
                        MovieFile.IsVideo_TS = True
                    ElseIf FileUtils.isBDRip(_tmpMovie.MoviePath) Then
                        MovieFile.Parent = Directory.GetParent(Directory.GetParent(Directory.GetParent(_tmpMovie.MoviePath).FullName).FullName).Name
                        If MovieFile.BasePath = Directory.GetParent(Directory.GetParent(Directory.GetParent(_tmpMovie.MoviePath).FullName).FullName).FullName Then
                            MovieFile.OldPath = String.Empty
                            MovieFile.BasePath = Directory.GetParent(MovieFile.BasePath).FullName
                        Else
                            MovieFile.OldPath = Directory.GetParent(Directory.GetParent(Directory.GetParent(Directory.GetParent(_tmpMovie.MoviePath).FullName).FullName).FullName).FullName.Replace(i, String.Empty)
                        End If
                        MovieFile.IsBDMV = True
                    Else
                        MovieFile.Parent = Directory.GetParent(_tmpMovie.MoviePath).Name
                        If MovieFile.BasePath = Directory.GetParent(_tmpMovie.MoviePath).FullName Then
                            MovieFile.OldPath = String.Empty
                            MovieFile.BasePath = Directory.GetParent(MovieFile.BasePath).FullName
                        Else
                            MovieFile.OldPath = Directory.GetParent(Directory.GetParent(_tmpMovie.MoviePath).FullName).FullName.Replace(i, String.Empty)
                        End If
                    End If
                End If
            Next
            ...
            If Not MovieFile.IsVideo_TS AndAlso Not MovieFile.IsBDMV Then
                MovieFile.FileName = StringUtils.CleanStackingMarkers(Path.GetFileNameWithoutExtension(_tmpMovie.MoviePath))
                Dim stackMark As String = Path.GetFileNameWithoutExtension(_tmpMovie.MoviePath).Replace(MovieFile.FileName, String.Empty).ToLower
                If Not stackMark = String.Empty AndAlso _tmpMovie.Title.ToLower.EndsWith(stackMark) Then
                    MovieFile.FileName = Path.GetFileNameWithoutExtension(_tmpMovie.MoviePath)
                End If
            ...
            If HaveBase(folderPattern) Then
                MovieFile.NewPath = ProccessPattern(MovieFile, If(_tmpMovie.UseFolder, folderPattern, "$D")).Trim
            Else
                MovieFile.NewPath = Path.Combine(MovieFile.OldPath, ProccessPattern(MovieFile, If(_tmpMovie.UseFolder, folderPattern, "$D")).Trim)
            End If
            ...
            Else
                Classes.Database.SaveMovie(_tmpMovie, EntityState.Modified)
                'Save NFO here too : Not String.IsNullOrEmpty(_tmpMovie.Movie.IMDBID                
            End If
        End Sub

        Public Shared Function SelectMPAA(ByVal _movies As Model.Movie) As String
                ...
                For Each f As FileFolderRenamer.FileRename In _movies
                    If f.IsRenamed Then
                        ...
                        If Not f.IsLocked Then
                            ...
                            If Not f.ID = -1 Then
                                _movieDB = Classes.Database.GetMovie(f.ID)
                                DoDB = True
                            Else
                                _movieDB = Nothing
                                DoDB = False
                            ...
                            If Not srcDir = destDir Then
                                ...
                                Try
                                    ...
                                    DoUpdate = True
                                Catch ex As Exception
                                    Master.eLog.WriteToErrorLog(ex.Message, "Dir: " & srcDir & " " & destDir, Languages._Error)
                                        ...
                                        For Each lFile As FileInfo In lFi.OrderBy(Function(s) s.Name)
                                            ...
                                            If Not srcFile = dstFile Then
                                                Try
                                                    ...
                                                    DoUpdate = True
                                                Catch ex As Exception
                                                    Master.eLog.WriteToErrorLog(ex.Message, "File " & srcFile & " " & dstFile, Languages._Error)
                                                    'Need to make some type of failure log
                                                End Try
                                            End If
                                        Next
                            ...
                            If DoDB AndAlso DoUpdate Then
                                ...
                                Classes.Database.SaveMovie(_movieDB, EntityState.Modified)
                                If Not f.IsSingle Then
        ...
        Private Shared Sub DoRenameSingle(ByVal _frename As FileRename, ByRef _movie As Model.Movie, ByVal BatchMode As Boolean, ByVal toNfo As Boolean, ByVal ShowError As Boolean)
            Try
                If Not _movie.Lock Then
                    ...
                    If Not _frename.IsVideo_TS AndAlso Not _frename.IsBDMV Then
                        If (Not _frename.NewFileName = _frename.FileName) OrElse (_frename.Path = String.Empty AndAlso Not _frename.NewPath = String.Empty) OrElse Not _movie.UseFolder Then
                            Dim di As DirectoryInfo
                    ...
                    Classes.Database.SaveMovie(_movie, EntityState.Modified)
                    'Save NFO based on toNFO value                    
        ...
        Private Shared Sub UpdateFaSPaths(ByRef _DBM As Model.Movie, ByVal newPath As String, ByVal oldFile As String, ByVal newFile As String)
            ...
            If Not String.IsNullOrEmpty(_DBM.MoviePath) Then _DBM.MoviePath = Path.Combine(newPath, Path.GetFileName(_DBM.MoviePath).Replace(oldFile, newFile))
