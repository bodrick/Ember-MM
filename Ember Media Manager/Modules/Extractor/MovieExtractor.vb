...
Public Class MovieExtractor
    ...
    Public Property CurrentMovie As Model.Movie
    ...
    Private Sub btnFrameLoad_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnFrameLoad.Click
        Try
            Using ffmpeg As New Process()
                ...
                ffmpeg.StartInfo.Arguments = String.Format("-ss 0 -i ""{0}"" -an -f rawvideo -vframes 1 -s 1280x720 -vcodec mjpeg -y ""{1}""", CurrentMovie.MoviePath, Path.Combine(Master.TempPath, "frame.jpg"))
    ...
    Private Sub GrabTheFrame()
        Try
            ...
            ffmpeg.StartInfo.Arguments = String.Format("-ss {0} -i ""{1}"" -vframes 1 -y ""{2}""", tbFrame.Value, CurrentMovie.MoviePath, Path.Combine(Master.TempPath, "frame.jpg"))
            ffmpeg.EnableRaisingEvents = False
            ffmpeg.StartInfo.UseShellExecute = False
            ffmpeg.StartInfo.CreateNoWindow = True
    ...
    Private Sub btnAutoGen_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAutoGen.Click
        Try
            If Convert.ToInt32(txtThumbCount.Text) > 0 Then
                pnlFrameProgress.Visible = True
                Me.Refresh()
                ThumbGenerator.CreateRandomThumbs(CurrentMovie, Convert.ToInt32(txtThumbCount.Text), True)
                pnlFrameProgress.Visible = False
