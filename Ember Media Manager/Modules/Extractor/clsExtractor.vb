...
Public Class ThumbGenerator
    ...
    Public Shared Function CreateRandomThumbs(ByVal mMovie As Model.Movie, ByVal ThumbCount As Integer, ByVal isEdit As Boolean) As String
    ...
    Private Class GeneratorThread
        ...
        Public WriteOnly Property Movie() As Model.Movie
            Set(ByVal value As Model.Movie)
                _movie = value
            End Set
        End Property
        ...
        Private Sub CreateRandom()
            Try
                Dim pExt As String = Path.GetExtension(_movie.MoviePath).ToLower
                ...
                Not pExt = ".bin" AndAlso Not pExt = ".cue" Then
                    ...
                    If _isedit Then
                        ...
                        eMovieFile = _movie.MoviePath
                    Else
                        If Master.eSettings.VideoTSParent AndAlso FileUtils.isVideoTS(_movie.MoviePath) Then
                            tPath = Path.Combine(Directory.GetParent(Directory.GetParent(_movie.MoviePath).FullName).FullName, "extrathumbs")
                            eMovieFile = FileUtils.GetLongestFromRip(_movie.MoviePath)
                        ElseIf Master.eSettings.VideoTSParent AndAlso FileUtils.isBDRip(_movie.MoviePath) Then
                            tPath = Path.Combine(Directory.GetParent(Directory.GetParent(Directory.GetParent(_movie.MoviePath).FullName).FullName).FullName, "extrathumbs")
                            eMovieFile = FileUtils.GetLongestFromRip(_movie.MoviePath)
                        Else
                            tPath = Path.Combine(Directory.GetParent(_movie.MoviePath).FullName, "extrathumbs")
                            If FileUtils.isVideoTS(_movie.MoviePath) OrElse FileUtils.isBDRip(_movie.MoviePath) Then
                                eMovieFile = FileUtils.GetLongestFromRip(_movie.MoviePath)
                            Else
                                eMovieFile = _movie.MoviePath
                            End If
