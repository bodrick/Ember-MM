...
Namespace Modules.Extras
    ...
    Public Class GenresEditor
                ...
                If cbLangs.SelectedItem.ToString = Master.eLang.GetString(2, "< All >") Then
                    For Each sett As xGenre In xmlGenres.listOfGenres
                        ...
                        If sett.Langs.Count = 0 Then
                            dgvGenres.Rows(i).DefaultCellStyle.ForeColor = Color.Red
                        End If
                    Next
                Else
        ...
        Private Sub ClearLangSelection()
            For Each r As DataGridViewRow In dgvLang.Rows.Cast(Of DataGridViewRow)()
        ...
        End Sub
        Private Sub dgvGenres_KeyDown(ByVal sender As Object, ByVal e As Windows.Forms.KeyEventArgs) Handles dgvGenres.KeyDown
        ...
        Private Sub dgvLang_CurrentCellDirtyStateChanged(ByVal sender As Object, ByVal e As EventArgs) Handles dgvLang.CurrentCellDirtyStateChanged
            Dim g As xGenre = DirectCast(dgvGenres.CurrentRow.Tag, xGenre)
            If Not g Is Nothing Then
                ...
                If g.Langs.Count > 0 AndAlso dgvLang.CurrentRow.DefaultCellStyle.ForeColor = Color.Red Then
                    dgvLang.CurrentRow.DefaultCellStyle.ForeColor = Color.Black
                End If
            End If
        End Sub
        Private Sub dgvLang_SelectionChanged(ByVal sender As Object, ByVal e As EventArgs) Handles dgvLang.SelectionChanged
        ...
        Private Sub cbLangs_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As EventArgs) Handles cbLangs.SelectedIndexChanged
            PopulateGenres()
        End Sub

        Private Sub btnAddGenre_Click(ByVal sender As System.Object, ByVal e As EventArgs) Handles btnAddGenre.Click
            Dim g As New xGenre
        ...
        Private Sub btnAddLang_Click(ByVal sender As System.Object, ByVal e As EventArgs) Handles btnAddLang.Click
            Dim s As String = InputBox(Languages.Enter_the_new_Language, Languages.New_Language)
            Dim i As Integer = dgvLang.Rows.Add(New Object() {False, s})
            dgvLang.CurrentCell = dgvLang.Rows(i).Cells(1)
            dgvLang.BeginEdit(True)            
        End Sub
        Private Sub btnRemoveGenre_Click(ByVal sender As System.Object, ByVal e As EventArgs) Handles btnRemoveGenre.Click
            If dgvGenres.SelectedCells.Count > 0 Then
                dgvGenres.Rows.RemoveAt(dgvGenres.SelectedCells(0).RowIndex)                
            End If
        End Sub

        Private Sub btnRemoveLang_Click(ByVal sender As System.Object, ByVal e As EventArgs) Handles btnRemoveLang.Click
            If MsgBox(Languages.This_will_remove_the_Language_from_all_Genres_Are_you_sure, MsgBoxStyle.YesNo, Languages.Remove_Language) = MsgBoxResult.Yes Then
                If dgvLang.SelectedRows.Count > 0 AndAlso Not dgvLang.CurrentRow.Cells(1).Value Is Nothing Then
                    ...
                    For Each g As xGenre In xmlGenres.listOfGenres
                        If g.Langs.Contains(lang) Then
                            ...
                            If g.Langs.Count = 0 Then
                                For Each d As DataGridViewRow In dgvGenres.Rows.Cast(Of DataGridViewRow)()
        ...
        Private Sub btnChangeImg_Click(ByVal sender As System.Object, ByVal e As EventArgs) Handles btnChangeImg.Click
            Try
...
#Region "Nested Types"
        ...
        Class xGenres
            ...
            Public Sub Save(ByVal fpath As String)
                For i As Integer = 0 To listOfGenres.Count - 1
                    listOfGenres(i).language = Strings.Join(Me.listOfGenres(i).Langs.ToArray, "|")
                Next
...
#End Region 'Nested Types
        ...
        Protected Overrides Sub LoadResources()
            btnAddGenre.Text = Languages.Add
            btnAddLang.Text = Languages.Add
            btnRemoveGenre.Text = Languages.Remove
            btnRemoveLang.Text = Languages.Remove
            btnChangeImg.Text = Languages.Change
            GroupBox1.Text = Languages.Images
            Label1.Text = Languages.Genres_Filter
            dgvGenres.Columns(0).HeaderText = Languages.Genres
            dgvLang.Columns(1).HeaderText = Languages.Languages                        
        End Sub
    End Class
End Namespace