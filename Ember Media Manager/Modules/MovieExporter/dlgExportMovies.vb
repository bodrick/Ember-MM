...
#Region "Fields"
        ...
        Private _movies As New List(Of Model.Movie)
...
#Region "Methods"
        ...
        Private Sub BuildHTML(ByVal bSearch As Boolean, ByVal strFilter As String, ByVal strIn As String, ByVal template As String, ByVal doNavigate As Boolean)
            Try
                ...
                FilterMovies.Clear()
                For Each _curMovie As Model.Movie In _movies
                    ...
                    If Not IsNothing(_curMovie.FileInfo) Then
                        If _curMovie.FileInfo.StreamDetails.Video.Count > 0 Then
                            tVid = MediaInfo.GetBestVideo(_curMovie.FileInfo)
                            tRes = MediaInfo.GetResFromDimensions(tVid)
                            _vidDimensions = MediaInfo.GetDimensionsFromVideo(tVid)
                        ...
                        If _curMovie.FileInfo.StreamDetails.Audio.Count > 0 Then
                            tAud = MediaInfo.GetBestAudio(_curMovie.FileInfo, False)
                    ...
                    If bSearch Then
                        ...
                        Else
                            If (strIn = Master.eLang.GetString(12, "Video Flag") AndAlso StringUtils.Wildcard.IsMatch(_vidDetails, strFilter)) OrElse _
                               ...
                               (strIn = Master.eLang.GetString(21, "Title", True) AndAlso StringUtils.Wildcard.IsMatch(_curMovie.Title, strFilter)) OrElse _
                               (strIn = Master.eLang.GetString(278, "Year", True) AndAlso StringUtils.Wildcard.IsMatch(_curMovie.Year, strFilter)) Then
                    ...
                    If Not String.IsNullOrEmpty(_curMovie.Title) Then
                        row = row.Replace("<$MOVIENAME>", StringUtils.HtmlEncode(_curMovie.Title))
                    Else
                        row = row.Replace("<$MOVIENAME>", StringUtils.HtmlEncode(_curMovie.ListTitle))
                    End If
                    row = row.Replace("<$ORIGINALTITLE>", StringUtils.HtmlEncode(_curMovie.OriginalTitle))
                    row = row.Replace("<$ACTORS>", StringUtils.HtmlEncode(Functions.ListToStringWithSeparator(Of String)(_curMovie.MoviesActors, ",")))
                    row = row.Replace("<$DIRECTOR>", StringUtils.HtmlEncode(_curMovie.Director))
                    row = row.Replace("<$CERTIFICATION>", StringUtils.HtmlEncode(_curMovie.Certification))
                    row = row.Replace("<$IMDBID>", StringUtils.HtmlEncode(_curMovie.Imdb))
                    row = row.Replace("<$MPAA>", StringUtils.HtmlEncode(_curMovie.MPAA))
                    row = row.Replace("<$RELEASEDATE>", StringUtils.HtmlEncode(_curMovie.ReleaseDate))
                    row = row.Replace("<$RUNTIME>", StringUtils.HtmlEncode(_curMovie.Runtime))
                    row = row.Replace("<$TAGLINE>", StringUtils.HtmlEncode(_curMovie.Tagline))
                    row = row.Replace("<$RATING>", StringUtils.HtmlEncode(_curMovie.Rating))
                    row = row.Replace("<$VOTES>", StringUtils.HtmlEncode(_curMovie.Votes))
                    row = row.Replace("<$LISTTITLE>", StringUtils.HtmlEncode(_curMovie.ListTitle))
                    row = row.Replace("<$YEAR>", _curMovie.Year)
                    row = row.Replace("<$COUNTRY>", StringUtils.HtmlEncode(_curMovie.Country))
                    row = row.Replace("<$COUNT>", counter.ToString)
                    row = row.Replace("<$FILENAME>", StringUtils.HtmlEncode(Path.GetFileName(_curMovie.MoviePath)))
                    row = row.Replace("<$DIRNAME>", StringUtils.HtmlEncode(Path.GetDirectoryName(_curMovie.MoviePath)))
                    row = row.Replace("<$OUTLINE>", StringUtils.HtmlEncode(_curMovie.Outline))
                    row = row.Replace("<$PLOT>", StringUtils.HtmlEncode(_curMovie.Plot))
                    row = row.Replace("<$GENRES>", StringUtils.HtmlEncode(_curMovie.Genre))
                    row = row.Replace("<$VIDEO>", _vidDetails)
                    row = row.Replace("<$VIDEO_DIMENSIONS>", _vidDimensions)
                    row = row.Replace("<$AUDIO>", _audDetails)
                    row = row.Replace("<$SIZE>", StringUtils.HtmlEncode(MovieSize(_curMovie.MoviePath).ToString))
                    ...
                    Using SQLreader As SQLite.SQLiteDataReader = SQLNewcommand.ExecuteReader()
                        If SQLreader.HasRows Then
                            While SQLreader.Read()
                                ...
                                _tmpMovie = Classes.Database.GetMovie(_ID)                                
        ...
        Private Sub bwSaveAll_DoWork(ByVal sender As System.Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles bwSaveAll.DoWork
            Try
                ...
                If Not DontSaveExtra Then
                    FileUtils.DeleteDirectory(Me.TempPath)
        ...
        Private Sub ExportFanart(ByVal fpath As String)
            Try
                ...
                For Each _curMovie As Model.Movie In _movies.Where(Function(y) FilterMovies.Contains(y.ID))
                    Try
        ...
        Private Function GetAVImages(ByVal AVMovie As Model.Movie, ByVal line As String) As String
            If APIXML.lFlags.Count > 0 Then
                Try
                    Dim fiAV As MediaInfo.Fileinfo = AVMovie.FileInfo
                    Dim tVideo As MediaInfo.Video = MediaInfo.GetBestVideo(fiAV)
                    Dim tAudio As MediaInfo.Audio = MediaInfo.GetBestAudio(fiAV, False)

                    Dim vresFlag As APIXML.Flag = APIXML.lFlags.FirstOrDefault(Function(f) f.Name = MediaInfo.GetResFromDimensions(tVideo).ToLower AndAlso f.Type = APIXML.FlagType.VideoResolution)
        ...
        Function MovieSize(ByVal spath As String) As Long
            ...
            If StringUtils.IsStacked(Path.GetFileNameWithoutExtension(spath), True) OrElse FileUtils.isVideoTS(spath) OrElse FileUtils.isBDRip(spath) Then
                Try
