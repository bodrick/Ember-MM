...
#Region "Fields"
    ...
    Private tMovie As New Model.Movie
...
#Region "Methods"
    ...
    Public Sub PreLoad(ByVal mMovie As Model.Movie, ByVal _DLType As Enums.ImageType, Optional ByVal _isEdit As Boolean = False)
        tMovie = mMovie
        DLType = _DLType
        isEdit = _isEdit
        PreDL = True
    ...
    Public Overloads Function ShowDialog(ByVal mMovie As Model.Movie, ByVal _DLType As Enums.ImageType, Optional ByVal _isEdit As Boolean = False) As Containers.ImgResult
        '//
    ...
    Private Sub btnPreview_Click(ByVal sender As System.Object, ByVal e As EventArgs) Handles btnPreview.Click
        Try
            ...
            Select Case True
                Case rbXLarge.Checked
                    If Master.eSettings.UseImgCache Then
                        tImage = Image.FromFile(Path.Combine(CachePath, String.Concat("poster_(original)_(url=", rbXLarge.Tag, ").jpg")))
                    Else
                        tImage = Classes.Http.DownloadImage(rbXLarge.Tag.ToString)
                    End If
                Case rbLarge.Checked
                    If Master.eSettings.UseImgCache Then
                        tImage = Image.FromFile(Path.Combine(CachePath, String.Concat("poster_(mid)_(url=", rbLarge.Tag, ").jpg")))
                    Else
                        tImage = Classes.Http.DownloadImage(rbLarge.Tag.ToString)
                    End If
                Case rbMedium.Checked
                    If Master.eSettings.UseImgCache Then
                        tImage = Image.FromFile(Path.Combine(CachePath, String.Concat("poster_(cover)_(url=", rbMedium.Tag, ").jpg")))
                    Else
                        tImage = Classes.Http.DownloadImage(rbMedium.Tag.ToString)
                    End If
                Case rbSmall.Checked
                    If Master.eSettings.UseImgCache Then
                        tImage = Image.FromFile(Path.Combine(CachePath, String.Concat("poster_(thumb)_(url=", rbSmall.Tag, ").jpg")))
                    Else
                        tImage = Classes.Http.DownloadImage(rbSmall.Tag.ToString)
                    End If
            End Select
            ...
            If Not IsNothing(tImage) Then
                Dim imgView As New dlgImgView
                imgView.ShowDialog(tImage)
            End If
        Catch ex As Exception
            pnlSinglePic.Visible = False
            Master.eLog.WriteToErrorLog(ex.Message, ex.StackTrace, Languages._Error)
    ...
    Private Sub GetFanart()
        Try
            ...
            If NoneFound Then
                ...
                Refresh()
                Application.DoEvents()
                'Dim _images As List(Of MediaContainers.Image) = (From t In tMovie.Fanart.Thumb Select New MediaContainers.Image With {.URL = t.Text}).ToList()
    ...
    Private Sub GetPosters()
        Try
            ...
            If NoneFound Then
                ...
                Refresh()
                Application.DoEvents()
                'Dim _images As List(Of MediaContainers.Image) = (From t In tMovie. Select New MediaContainers.Image With {.URL = t}).ToList()
                'ImagesDownloaded(_images)
            End If
        Catch ex As Exception
            Master.eLog.WriteToErrorLog(ex.Message, ex.StackTrace, Languages._Error)
        ...
        Try
            ...
            If DLType = Enums.ImageType.Fanart Then
                ...
                If isChecked Then
                    ...
                    Else
                        If Master.eSettings.VideoTSParent AndAlso FileUtils.isVideoTS(tMovie.MoviePath) Then
                            extraPath = Path.Combine(Directory.GetParent(Directory.GetParent(tMovie.MoviePath).FullName).FullName, "extrathumbs")
                        ElseIf Master.eSettings.VideoTSParent AndAlso FileUtils.isBDRip(tMovie.MoviePath) Then
                            extraPath = Path.Combine(Directory.GetParent(Directory.GetParent(Directory.GetParent(tMovie.MoviePath).FullName).FullName).FullName, "extrathumbs")
                        Else
                            extraPath = Path.Combine(Directory.GetParent(tMovie.MoviePath).FullName, "extrathumbs")
                        End If
    ...
    Private Sub SetUp()
        Try
            ...
            If DLType = Enums.ImageType.Posters Then
                Text = String.Concat(Master.eLang.GetString(39, "Select Poster - "), If(Not String.IsNullOrEmpty(tMovie.Title), tMovie.Title, tMovie.ListTitle))
            Else
                Text = String.Concat(Master.eLang.GetString(40, "Select Fanart - "), If(Not String.IsNullOrEmpty(tMovie.Title), tMovie.Title, tMovie.ListTitle))
                pnlDLStatus.Height = 75
                pnlDLStatus.Top = 207

                If Master.eSettings.AutoET Then
                    ETHashes = HashFile.CurrentETHashes(tMovie.MoviePath)
                End If
            End If

            CachePath = String.Concat(Master.TempPath, Path.DirectorySeparatorChar, tMovie.Imdb, Path.DirectorySeparatorChar, If(DLType = Enums.ImageType.Posters, "posters", "fanart"))
