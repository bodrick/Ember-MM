...
#Region "Methods"
        ...
        Public Sub GetMovieFolderContents(ByRef Movie As MovieContainer)
            ...
            Try
                If Master.eSettings.VideoTSParent AndAlso FileUtils.isVideoTS(Movie.Filename) Then
                    isYAMJ = True
                    ...
                    End If
                ElseIf Master.eSettings.VideoTSParent AndAlso FileUtils.isBDRip(Movie.Filename) Then
                    isYAMJ = True
        ...
        Public Sub GetShowFolderContents(ByRef tShow As TVShowContainer)
            ...
            Try
                ...
                If String.IsNullOrEmpty(tShow.Poster) AndAlso Master.eSettings.ShowTBN Then
                    fName = Path.Combine(parPath, String.Concat(FileUtils.GetDirectory(parPath), ".tbn"))
                ...
                If String.IsNullOrEmpty(tShow.Poster) AndAlso Master.eSettings.ShowJPG Then
                    fName = Path.Combine(parPath, String.Concat(FileUtils.GetDirectory(parPath), ".jpg"))
                    tShow.Poster = fList.FirstOrDefault(Function(s) s.ToLower = fName.ToLower)
                End If
                ...
                If String.IsNullOrEmpty(tShow.Fanart) AndAlso Master.eSettings.ShowDashFanart Then
                    fName = Path.Combine(parPath, String.Concat(FileUtils.GetDirectory(parPath), "-fanart.jpg"))
                    tShow.Fanart = fList.FirstOrDefault(Function(s) s.ToLower = fName.ToLower)
                End If
        ...
        Public Sub LoadMovie(ByVal mContainer As MovieContainer)
            Dim tmpMovieDB As New Model.Movie
            Try
                ...
                If Not String.IsNullOrEmpty(mContainer.Nfo) Then
                    ...
                    'If Not tmpMovieDB.FileInfoSpecified AndAlso Not String.IsNullOrEmpty(tmpMovieDB.Title) AndAlso Master.eSettings.ScanMediaInfo Then
                ...
                If String.IsNullOrEmpty(tmpMovieDB.Title) Then
                    'no title so assume it's an invalid nfo, clear nfo path if exists
                    mContainer.Nfo = String.Empty

                    If FileUtils.isVideoTS(mContainer.Filename) Then
                        tmpMovieDB.ListTitle = StringUtils.FilterName(Directory.GetParent(Directory.GetParent(mContainer.Filename).FullName).Name)
                        tmpMovieDB.Title = StringUtils.FilterName(Directory.GetParent(Directory.GetParent(mContainer.Filename).FullName).Name, False)
                        If String.IsNullOrEmpty(tmpMovieDB.Title) Then
                            tmpMovieDB.ListTitle = Directory.GetParent(Directory.GetParent(mContainer.Filename).FullName).Name
                            tmpMovieDB.Title = Directory.GetParent(Directory.GetParent(mContainer.Filename).FullName).Name
                        End If
                    ElseIf FileUtils.isBDRip(mContainer.Filename) Then
                        ...
                        tmpMovieDB.Title = StringUtils.FilterName(Directory.GetParent(Directory.GetParent(Directory.GetParent(mContainer.Filename).FullName).FullName).Name, False)
                        If String.IsNullOrEmpty(tmpMovieDB.Title) Then
                            ...
                            tmpMovieDB.Title = Directory.GetParent(Directory.GetParent(Directory.GetParent(mContainer.Filename).FullName).FullName).Name
                        End If
                    Else
                        If mContainer.UseFolder AndAlso mContainer.isSingle Then
                            ...
                            tmpMovieDB.Title = StringUtils.FilterName(Directory.GetParent(mContainer.Filename).Name, False)
                            If String.IsNullOrEmpty(tmpMovieDB.Title) Then
                                tmpMovieDB.ListTitle = Directory.GetParent(mContainer.Filename).Name
                                tmpMovieDB.Title = Directory.GetParent(mContainer.Filename).Name
                            End If
                        Else
                            tmpMovieDB.ListTitle = StringUtils.FilterName(Path.GetFileNameWithoutExtension(mContainer.Filename))
                            tmpMovieDB.Title = StringUtils.FilterName(Path.GetFileNameWithoutExtension(mContainer.Filename), False)
                            If String.IsNullOrEmpty(tmpMovieDB.Title) Then
                                tmpMovieDB.ListTitle = Path.GetFileNameWithoutExtension(mContainer.Filename)
                                tmpMovieDB.Title = Path.GetFileNameWithoutExtension(mContainer.Filename)
                            End If
                        End If
                    End If

                    If String.IsNullOrEmpty(tmpMovieDB.SortTitle) Then tmpMovieDB.SortTitle = tmpMovieDB.ListTitle

                Else
                    Dim tTitle As String = StringUtils.FilterTokens(tmpMovieDB.Title)
                    If String.IsNullOrEmpty(tmpMovieDB.SortTitle) Then tmpMovieDB.SortTitle = tTitle
                    If Master.eSettings.DisplayYear AndAlso Not String.IsNullOrEmpty(tmpMovieDB.Year) Then
                        tmpMovieDB.ListTitle = String.Format("{0} ({1})", tTitle, tmpMovieDB.Year)
                    Else
                        tmpMovieDB.ListTitle = StringUtils.FilterTokens(tmpMovieDB.Title)
                    End If
                End If
                ...
                If Not String.IsNullOrEmpty(tmpMovieDB.ListTitle) Then
                    ...
                    tmpMovieDB.MoviePath = mContainer.Filename
                    tmpMovieDB.UseFolder = mContainer.isSingle
                    ...
                    ElseIf String.IsNullOrEmpty(tmpMovieDB.FileSource) AndAlso AdvancedSettings.GetBooleanSetting("MediaSourcesByExtension", False, "*EmberAPP") Then
                        tmpMovieDB.FileSource = AdvancedSettings.GetSetting(String.Concat("MediaSourcesByExtension:", Path.GetExtension(tmpMovieDB.MoviePath)), String.Empty, "*EmberAPP")
                    End If
                    tmpMovieDB.Lock = False
                    tmpMovieDB.Mark = Master.eSettings.MarkNew
                    'Do the Save
                    Classes.Database.SaveMovie(tmpMovieDB, EntityState.Added)

                    Me.bwPrelim.ReportProgress(0, New ProgressValue With {.Type = 0, .Message = tmpMovieDB.Title})
                End If
            Catch ex As Exception
                Master.eLog.WriteToErrorLog(ex.Message, ex.StackTrace, "Error")
            End Try
                ...
                If lFi.Count > 0 Then
                    ...
                    If Master.eSettings.AutoDetectVTS AndAlso autoCheck Then
                        If lFi.Where(Function(s) s.Name.ToLower = "index.bdmv").Count > 0 Then
                            bdmvSingle = True
                            tFile = FileUtils.GetLongestFromRip(sPath, True)
        ...
        Private Sub LoadShow(ByVal TVContainer As TVShowContainer)
            ...
            '                'tmpTVDB = NFO.LoadTVShowFromNFO(TVContainer.Nfo)
            '            Else
            '                tmpTVDB = New Model.TVShow
            '            End If

            '            If String.IsNullOrEmpty(tmpTVDB.Title) Then
            '                'no title so assume it's an invalid nfo, clear nfo path if exists
            '                TVContainer.Nfo = String.Empty
            '                tmpTVDB.Title = StringUtils.FilterTVShowName(FileUtils.GetDirectory(TVContainer.ShowPath))

            '                'everything was filtered out... just set to directory name
            '                If String.IsNullOrEmpty(tmpTVDB.Title) Then tmpTVDB.Title = FileUtils.GetDirectory(TVContainer.ShowPath)
            '            End If

            '            tmpTVDB.TVShowPath = TVContainer.ShowPath
            '            tmpTVDB.NfoPath = TVContainer.Nfo
            '            tmpTVDB.PosterPath = TVContainer.Poster
            '            tmpTVDB.FanartPath = TVContainer.Fanart
            '            tmpTVDB.Lock = False
            '            tmpTVDB.Mark = False
            '            tmpTVDB.Source = TVContainer.Source
            '            tmpTVDB.Ordering = Master.eSettings.OrderDefault
            '            tmpTVDB.Language = Master.eSettings.TVDBLanguage

            '            Classes.Database.SaveTVShow(tmpTVDB, EntityState.Added)
            '        Else
            '            tmpTVDB = Master.DB.LoadTVFullShowFromDB(Convert.ToInt64(htTVShows.Item(TVContainer.ShowPath.ToLower)))
            '        End If
            '        If tmpTVDB.TVDB > -1 Then
