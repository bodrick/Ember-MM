...
#Region "Methods"
        ...
        Public Sub DeleteAllSeasonPosters(ByVal mShow As Model.TVEp)
            Try
                Dim tPath As String = mShow.TVShow.TVShowPath
        ...
        Public Sub DeleteEpFanart(ByVal mShow As Model.TVEp)
            Try
                'Get TVEpisode Path
                'Delete(String.Concat(FileUtils.RemoveExtFromPath(mShow.ShowPath), "-fanart.jpg"))
                'Delete(String.Concat(FileUtils.RemoveExtFromPath(mShow.ShowPath), ".fanart.jpg"))
            Catch ex As Exception
                Master.eLog.WriteToErrorLog(ex.Message, ex.StackTrace, Languages._Error)
            End Try
        ...
        Public Sub DeleteEpPosters(ByVal mShow As Model.TVEp)
            Try
                'Get TVEpisode Path
                'Dim tPath As String = FileUtils.RemoveExtFromPath(mShow.Filename)
        ...
        Public Sub DeleteFanart(ByVal mMovie As Model.Movie)
            Try
                Dim tPath As String = Directory.GetParent(mMovie.MoviePath).FullName
                ...
                If Master.eSettings.VideoTSParent AndAlso FileUtils.isVideoTS(mMovie.MoviePath) Then
                    Delete(String.Concat(Path.Combine(Directory.GetParent(tPath).FullName, Directory.GetParent(tPath).Name), ".fanart.jpg"))
                ElseIf Master.eSettings.VideoTSParent AndAlso FileUtils.isBDRip(mMovie.MoviePath) Then
                ...
                Else
                    If mMovie.UseFolder Then
                        Delete(Path.Combine(tPath, "fanart.jpg"))
                    End If

                    If FileUtils.isVideoTS(mMovie.MoviePath) Then
                        Delete(Path.Combine(tPath, "video_ts-fanart.jpg"))
                        Delete(Path.Combine(tPath, "video_ts.fanart.jpg"))
                    ElseIf FileUtils.isBDRip(mMovie.MoviePath) Then
                        Delete(Path.Combine(tPath, "index-fanart.jpg"))
                        Delete(Path.Combine(tPath, "index.fanart.jpg"))
                    Else
                        Dim fPath As String = Path.Combine(tPath, Path.GetFileNameWithoutExtension(mMovie.MoviePath))
        ...
        Public Sub DeletePosters(ByVal mMovie As Model.Movie)
            Try
                Dim tPath As String = Directory.GetParent(mMovie.MoviePath).FullName
                Dim params As New List(Of Object)(New Object() {mMovie})
                ...
                Else
                    ...
                    Else
                        Dim pPath As String = Path.Combine(tPath, Path.GetFileNameWithoutExtension(mMovie.MoviePath))
        ...
        Public Sub DeleteSeasonFanart(ByVal mShow As Model.TVSeason)
            Try
                Dim tPath As String = String.Empty

                Try
                    tPath = Functions.GetSeasonDirectoryFromShowPath(mShow.TVShow.TVShowPath, mShow.Season)
                Catch ex As Exception
                End Try

                If Not String.IsNullOrEmpty(tPath) Then
                    Delete(Path.Combine(tPath, String.Concat(FileUtils.GetDirectory(tPath), ".fanart.jpg")))
                    Delete(Path.Combine(tPath, String.Concat(FileUtils.GetDirectory(tPath), "-fanart.jpg")))
                    Delete(Path.Combine(tPath, "Fanart.jpg"))
                End If
        ...
        Public Sub DeleteSeasonPosters(ByVal mShow As Model.TVSeason)
            Try
                Dim tPath As String = String.Empty
                ...
                If Not String.IsNullOrEmpty(tPath) Then
                    Delete(Path.Combine(tPath, "Poster.tbn"))
                    Delete(Path.Combine(tPath, "Poster.jpg"))
                    Delete(Path.Combine(tPath, String.Concat(FileUtils.GetDirectory(tPath), ".tbn")))
                    Delete(Path.Combine(tPath, String.Concat(FileUtils.GetDirectory(tPath), ".jpg")))
                    Delete(Path.Combine(tPath, "Folder.jpg"))
                End If

                If mShow.Season = 0 Then
                    Delete(Path.Combine(mShow.TVShow.TVShowPath, "season-specials.tbn"))
                    Delete(Path.Combine(mShow.TVShow.TVShowPath, "season-specials.tbn"))
                Else
                    Delete(Path.Combine(mShow.TVShow.TVShowPath, String.Format("season{0}.tbn", mShow.Season)))
                    Delete(Path.Combine(mShow.TVShow.TVShowPath, String.Format("season{0}.tbn", mShow.Season.ToString.PadLeft(2, Convert.ToChar("0")))))
                End If
        ...
        Public Sub DeleteShowFanart(ByVal mShow As Model.TVEp)
            Try
                Delete(Path.Combine(mShow.TVShow.TVShowPath, "fanart.jpg"))
                Delete(Path.Combine(mShow.TVShow.TVShowPath, String.Concat(FileUtils.GetDirectory(mShow.TVShow.TVShowPath), "-fanart.jpg")))
                Delete(Path.Combine(mShow.TVShow.TVShowPath, String.Concat(FileUtils.GetDirectory(mShow.TVShow.TVShowPath), ".fanart.jpg")))
            Catch ex As Exception
                Master.eLog.WriteToErrorLog(ex.Message, ex.StackTrace, Languages._Error)
            End Try
        ...
        Public Sub DeleteShowPosters(ByVal mShow As Model.TVEp)
            Try
                Dim tPath As String = mShow.TVShow.TVShowPath
        ...
        Public Function SaveAsAllSeasonPoster(ByVal mShow As Model.TVEp) As String
            ...
            Try
                ...
                If Master.eSettings.SeasonAllJPG Then
                    pPath = Path.Combine(mShow.TVShow.TVShowPath, "season-all.jpg")
                ...
                If Master.eSettings.SeasonAllTBN Then
                    pPath = Path.Combine(mShow.TVShow.TVShowPath, "season-all.tbn")
                    If Not File.Exists(pPath) OrElse (IsEdit OrElse Master.eSettings.OverwriteAllSPoster) Then
                        Save(pPath, Master.eSettings.AllSPosterQuality)
                        strReturn = pPath
                    End If
        ...
        Public Function SaveAsEpFanart(ByVal mShow As Model.TVEp) As String
            Dim strReturn As String = String.Empty
            ...
            Try
                ...
                If Master.eSettings.EpisodeDotFanart Then
                    'Figure out TV Show Path here
                    tPath = String.Concat(FileUtils.RemoveExtFromPath(mShow.TVEpPathID), ".fanart.jpg")
        ...
        Public Function SaveAsEpPoster(ByVal mShow As Model.TVEp) As String
            Dim strReturn As String = String.Empty
            ...
            Try
                ...
                If Master.eSettings.EpisodeJPG Then
                    'Get Path again here
                    pPath = String.Concat(FileUtils.RemoveExtFromPath(mShow.TVEpPathID), ".jpg")
                ...
                If Master.eSettings.EpisodeTBN Then
                    pPath = String.Concat(FileUtils.RemoveExtFromPath(mShow.TVEpPathID), ".tbn")
                    If Not File.Exists(pPath) OrElse (IsEdit OrElse Master.eSettings.OverwriteEpPoster) Then
                        Save(pPath, Master.eSettings.EpPosterQuality)
                        strReturn = pPath
                    End If
        ...
        Public Function SaveAsFanart(ByVal mMovie As Model.Movie) As String
            Dim strReturn As String = String.Empty
            ...
            Try
                ...
                If Master.eSettings.VideoTSParent AndAlso FileUtils.isVideoTS(mMovie.MoviePath) Then
                    fPath = String.Concat(Path.Combine(Directory.GetParent(Directory.GetParent(mMovie.MoviePath).FullName).FullName, Directory.GetParent(Directory.GetParent(mMovie.MoviePath).FullName).Name), ".fanart.jpg")
                    ...
                    End If
                ElseIf Master.eSettings.VideoTSParent AndAlso FileUtils.isBDRip(mMovie.MoviePath) Then
                    fPath = String.Concat(Path.Combine(Directory.GetParent(Directory.GetParent(Directory.GetParent(mMovie.MoviePath).FullName).FullName).FullName, Directory.GetParent(Directory.GetParent(Directory.GetParent(mMovie.MoviePath).FullName).FullName).Name), ".fanart.jpg")
                    If Not File.Exists(fPath) OrElse (IsEdit OrElse Master.eSettings.OverwritePoster) Then
                        Save(fPath, Master.eSettings.FanartQuality)
                        strReturn = fPath
                        If Master.eSettings.AutoBD AndAlso Directory.Exists(Master.eSettings.BDPath) Then
                    ...
                    End If
                Else
                    Dim tmpName As String = Path.GetFileNameWithoutExtension(mMovie.MoviePath)
                    fPath = Path.Combine(Directory.GetParent(mMovie.MoviePath).FullName, tmpName)

                    If Master.eSettings.MovieNameDotFanartJPG AndAlso (Not mMovie.UseFolder OrElse Not Master.eSettings.MovieNameMultiOnly) Then
                        If FileUtils.isVideoTS(mMovie.MoviePath) Then
                            tPath = Path.Combine(Directory.GetParent(mMovie.MoviePath).FullName, "video_ts.fanart.jpg")
                        ElseIf FileUtils.isBDRip(mMovie.MoviePath) Then
                            tPath = Path.Combine(Directory.GetParent(mMovie.MoviePath).FullName, "index.fanart.jpg")
                        Else
                        ...
                        If Not File.Exists(tPath) OrElse (IsEdit OrElse Master.eSettings.OverwriteFanart) Then
                            ...
                            If Master.eSettings.AutoBD AndAlso Directory.Exists(Master.eSettings.BDPath) Then
                                If FileUtils.isVideoTS(mMovie.MoviePath) Then
                                    Save(Path.Combine(Master.eSettings.BDPath, String.Concat(Directory.GetParent(Directory.GetParent(mMovie.MoviePath).FullName).Name, "-fanart.jpg")), Master.eSettings.FanartQuality)
                                ElseIf FileUtils.isBDRip(mMovie.MoviePath) Then
                                    Save(Path.Combine(Master.eSettings.BDPath, String.Concat(Directory.GetParent(Directory.GetParent(Directory.GetParent(mMovie.MoviePath).FullName).FullName).Name, "-fanart.jpg")), Master.eSettings.FanartQuality)
                                Else
                    ...
                    If Master.eSettings.MovieNameFanartJPG AndAlso (Not mMovie.UseFolder OrElse Not Master.eSettings.MovieNameMultiOnly) Then
                        If FileUtils.isVideoTS(mMovie.MoviePath) Then
                            tPath = Path.Combine(Directory.GetParent(mMovie.MoviePath).FullName, "video_ts-fanart.jpg")
                        ElseIf FileUtils.isBDRip(mMovie.MoviePath) Then
                            tPath = Path.Combine(Directory.GetParent(mMovie.MoviePath).FullName, "index-fanart.jpg")
                    ...
                    If Master.eSettings.FanartJPG AndAlso mMovie.UseFolder Then
                        tPath = Path.Combine(Directory.GetParent(mMovie.MoviePath).FullName, "fanart.jpg")
                        If Not File.Exists(tPath) OrElse (IsEdit OrElse Master.eSettings.OverwriteFanart) Then
                            Save(tPath, Master.eSettings.FanartQuality)
                            strReturn = tPath
                            If Master.eSettings.AutoBD AndAlso Directory.Exists(Master.eSettings.BDPath) Then
        ...
        Public Function SaveAsPoster(ByVal mMovie As Model.Movie) As String
            Dim strReturn As String = String.Empty
            ...
            Try
                ...
                If Master.eSettings.VideoTSParent AndAlso FileUtils.isVideoTS(mMovie.MoviePath) Then
                    ...
                    If Master.eSettings.MovieNameJPG OrElse Master.eSettings.MovieJPG OrElse Master.eSettings.FolderJPG OrElse Master.eSettings.PosterJPG Then
                        pPath = String.Concat(Path.Combine(Directory.GetParent(Directory.GetParent(mMovie.MoviePath).FullName).FullName, Directory.GetParent(Directory.GetParent(mMovie.MoviePath).FullName).Name), ".jpg")
                    ...
                    ElseIf Master.eSettings.MovieNameTBN OrElse Master.eSettings.MovieTBN OrElse Master.eSettings.PosterTBN Then
                        pPath = String.Concat(Path.Combine(Directory.GetParent(Directory.GetParent(mMovie.MoviePath).FullName).FullName, Directory.GetParent(Directory.GetParent(mMovie.MoviePath).FullName).Name), ".tbn")
                        If Not File.Exists(pPath) OrElse (IsEdit OrElse Master.eSettings.OverwritePoster) Then
                            Save(pPath, Master.eSettings.PosterQuality)
                            strReturn = pPath
                        End If
                ...
                ElseIf Master.eSettings.VideoTSParent AndAlso FileUtils.isBDRip(mMovie.MoviePath) Then
                    ...
                    If Master.eSettings.MovieNameJPG OrElse Master.eSettings.MovieJPG OrElse Master.eSettings.FolderJPG OrElse Master.eSettings.PosterJPG Then
                        pPath = String.Concat(Path.Combine(Directory.GetParent(Directory.GetParent(Directory.GetParent(mMovie.MoviePath).FullName).FullName).FullName, Directory.GetParent(Directory.GetParent(Directory.GetParent(mMovie.MoviePath).FullName).FullName).Name), ".jpg")
                        If Not File.Exists(pPath) OrElse (IsEdit OrElse Master.eSettings.OverwritePoster) Then
                            Save(pPath, Master.eSettings.PosterQuality)
                            strReturn = pPath
                        End If
                    ElseIf Master.eSettings.MovieNameTBN OrElse Master.eSettings.MovieTBN OrElse Master.eSettings.PosterTBN Then
                        pPath = String.Concat(Path.Combine(Directory.GetParent(Directory.GetParent(Directory.GetParent(mMovie.MoviePath).FullName).FullName).FullName, Directory.GetParent(Directory.GetParent(Directory.GetParent(mMovie.MoviePath).FullName).FullName).Name), ".tbn")
                        If Not File.Exists(pPath) OrElse (IsEdit OrElse Master.eSettings.OverwritePoster) Then
                            Save(pPath, Master.eSettings.PosterQuality)
                            strReturn = pPath
                        End If
                ...
                Else
                    Dim tPath As String = String.Empty
                    Dim tmpName As String = Path.GetFileNameWithoutExtension(mMovie.MoviePath)
                    pPath = Path.Combine(Directory.GetParent(mMovie.MoviePath).FullName, tmpName)

                    If Master.eSettings.FolderJPG AndAlso mMovie.UseFolder Then
                        tPath = Path.Combine(Directory.GetParent(mMovie.MoviePath).FullName, "folder.jpg")
                    ...
                    If Master.eSettings.PosterJPG AndAlso mMovie.UseFolder Then
                        tPath = Path.Combine(Directory.GetParent(mMovie.MoviePath).FullName, "poster.jpg")
                        If Not File.Exists(tPath) OrElse (IsEdit OrElse Master.eSettings.OverwritePoster) Then
                            Save(tPath, Master.eSettings.PosterQuality)
                            strReturn = tPath
                        End If
                    ...
                    If Master.eSettings.PosterTBN AndAlso mMovie.UseFolder Then
                        tPath = Path.Combine(Directory.GetParent(mMovie.MoviePath).FullName, "poster.tbn")
                        If Not File.Exists(tPath) OrElse (IsEdit OrElse Master.eSettings.OverwritePoster) Then
                            Save(tPath, Master.eSettings.PosterQuality)
                            strReturn = tPath
                        End If
                    ...
                    If Master.eSettings.MovieNameJPG AndAlso (Not mMovie.UseFolder OrElse Not Master.eSettings.MovieNameMultiOnly) Then
                        If FileUtils.isVideoTS(mMovie.MoviePath) Then
                            tPath = Path.Combine(Directory.GetParent(mMovie.MoviePath).FullName, "video_ts.jpg")
                        ElseIf FileUtils.isBDRip(mMovie.MoviePath) Then
                            tPath = Path.Combine(Directory.GetParent(mMovie.MoviePath).FullName, "index.jpg")
                        Else
                    ...
                    If Master.eSettings.MovieJPG AndAlso mMovie.UseFolder Then
                        tPath = Path.Combine(Directory.GetParent(mMovie.MoviePath).FullName, "movie.jpg")
                        If Not File.Exists(tPath) OrElse (IsEdit OrElse Master.eSettings.OverwritePoster) Then
                            Save(tPath, Master.eSettings.PosterQuality)
                            strReturn = tPath
                        End If
                    ...
                    If Master.eSettings.MovieNameTBN AndAlso (Not mMovie.UseFolder OrElse Not Master.eSettings.MovieNameMultiOnly) Then
                        If FileUtils.isVideoTS(mMovie.MoviePath) Then
                            tPath = Path.Combine(Directory.GetParent(mMovie.MoviePath).FullName, "video_ts.tbn")
                        ElseIf FileUtils.isBDRip(mMovie.MoviePath) Then
                            tPath = Path.Combine(Directory.GetParent(mMovie.MoviePath).FullName, "index.tbn")
                        Else
                    ...
                    If Master.eSettings.MovieTBN AndAlso mMovie.UseFolder Then
                        tPath = Path.Combine(Directory.GetParent(mMovie.MoviePath).FullName, "movie.tbn")
                        If Not File.Exists(tPath) OrElse (IsEdit OrElse Master.eSettings.OverwritePoster) Then
                            Save(tPath, Master.eSettings.PosterQuality)
                            strReturn = tPath
                        End If
        ...
        Public Function SaveAsActorThumb(ByVal actor As MediaContainers.Person, ByVal fpath As String) As String
            ...
            Return tPath
        End Function
        Public Function SaveAsSeasonFanart(ByVal mShow As Model.TVSeason) As String
            Dim strReturn As String = String.Empty
            ...
            Try
                ...
                If Master.eSettings.SeasonFanartJPG OrElse Master.eSettings.SeasonDashFanart OrElse Master.eSettings.SeasonDotFanart Then
                    ...
                    Try
                        'tPath = Functions.GetSeasonDirectoryFromShowPath(mShow.TVShowPath, mShow.TVEp.Season)
                        'How do I get my season here?
                    Catch ex As Exception
                    End Try

                    If Not String.IsNullOrEmpty(tPath) Then
                        If Master.eSettings.SeasonDotFanart Then
                            pPath = Path.Combine(tPath, String.Concat(FileUtils.GetDirectory(tPath), ".fanart.jpg"))
        ...
        Public Function SaveAsSeasonPoster(ByVal mShow As Model.TVSeason) As String
            Dim strReturn As String = String.Empty
            ...
            Try
                   ...
                   Master.eSettings.SeasonNameJPG OrElse Master.eSettings.FolderJPG Then
                    ...
                    If Not String.IsNullOrEmpty(tPath) Then
                        ...
                        If Master.eSettings.SeasonNameTBN Then
                            pPath = Path.Combine(tPath, String.Concat(FileUtils.GetDirectory(tPath), ".tbn"))
                            If Not File.Exists(pPath) OrElse (IsEdit OrElse Master.eSettings.OverwriteSeasonPoster) Then
                                Save(pPath, Master.eSettings.SeasonPosterQuality)
                                strReturn = pPath
                            End If
                        ...
                        If Master.eSettings.SeasonNameJPG Then
                            pPath = Path.Combine(tPath, String.Concat(FileUtils.GetDirectory(tPath), ".jpg"))
                            If Not File.Exists(pPath) OrElse (IsEdit OrElse Master.eSettings.OverwriteSeasonPoster) Then
                                Save(pPath, Master.eSettings.SeasonPosterQuality)
                                strReturn = pPath
                            End If
                ...
                If Master.eSettings.SeasonX Then
                    If mShow.Season = 0 Then
                        pPath = Path.Combine(mShow.TVShow.TVShowPath, "season-specials.tbn")
                    Else
                        pPath = Path.Combine(mShow.TVShow.TVShowPath, String.Format("season{0}.tbn", mShow.Season))
                    End If
                    If Not File.Exists(pPath) OrElse (IsEdit OrElse Master.eSettings.OverwriteSeasonPoster) Then
                        Save(pPath, Master.eSettings.SeasonPosterQuality)
                        strReturn = pPath
                ...
                If Master.eSettings.SeasonXX Then
                    If mShow.Season = 0 Then
                        pPath = Path.Combine(mShow.TVShow.TVShowPath, "season-specials.tbn")
                    Else
                        pPath = Path.Combine(mShow.TVShow.TVShowPath, String.Format("season{0}.tbn", mShow.Season.ToString.PadLeft(2, Convert.ToChar("0"))))
                    End If
                    If Not File.Exists(pPath) OrElse (IsEdit OrElse Master.eSettings.OverwriteSeasonPoster) Then
                        Save(pPath, Master.eSettings.SeasonPosterQuality)
                        strReturn = pPath
        ...
        Public Function SaveAsShowFanart(ByVal mShow As Model.TVEp) As String
            Dim strReturn As String = String.Empty
            ...
            Try
                ...
                If Master.eSettings.ShowDotFanart Then
                    tPath = Path.Combine(mShow.TVShow.TVShowPath, String.Concat(FileUtils.GetDirectory(mShow.TVShow.TVShowPath), ".fanart.jpg"))
                ...
                If Master.eSettings.ShowFanartJPG Then
                    tPath = Path.Combine(mShow.TVShow.TVShowPath, "fanart.jpg")
                    If Not File.Exists(tPath) OrElse (IsEdit OrElse Master.eSettings.OverwriteShowFanart) Then
                        Save(tPath, Master.eSettings.ShowFanartQuality)
                        strReturn = tPath
                    End If
        ...
        Public Function SaveAsShowPoster(ByVal mShow As Model.TVEp) As String
            Dim strReturn As String = String.Empty
            ...
            Try
                ...
                If Master.eSettings.ShowPosterJPG Then
                    pPath = Path.Combine(mShow.TVShow.TVShowPath, "poster.jpg")
                ...
                If Master.eSettings.ShowPosterTBN Then
                    pPath = Path.Combine(mShow.TVShow.TVShowPath, "poster.tbn")
                    If Not File.Exists(pPath) OrElse (IsEdit OrElse Master.eSettings.OverwriteShowPoster) Then
                        Save(pPath, Master.eSettings.ShowPosterQuality)
                        strReturn = pPath
                    End If
                ...
                If Master.eSettings.ShowFolderJPG Then
                    pPath = Path.Combine(mShow.TVShow.TVShowPath, "folder.jpg")
                    If Not File.Exists(pPath) OrElse (IsEdit OrElse Master.eSettings.OverwriteShowPoster) Then
                        Save(pPath, Master.eSettings.ShowPosterQuality)
                        strReturn = pPath
                    End If
                ...
                If Master.eSettings.ShowJPG Then
                    pPath = Path.Combine(mShow.TVShow.TVShowPath, String.Concat(FileUtils.GetDirectory(mShow.TVShow.TVShowPath), ".jpg"))
                    If Not File.Exists(pPath) OrElse (IsEdit OrElse Master.eSettings.OverwriteShowPoster) Then
                        Save(pPath, Master.eSettings.ShowPosterQuality)
                        strReturn = pPath
                    End If
                ...
                If Master.eSettings.ShowTBN Then
                    pPath = Path.Combine(mShow.TVShow.TVShowPath, String.Concat(FileUtils.GetDirectory(mShow.TVShow.TVShowPath), ".tbn"))
                    If Not File.Exists(pPath) OrElse (IsEdit OrElse Master.eSettings.OverwriteShowPoster) Then
                        Save(pPath, Master.eSettings.ShowPosterQuality)
                        strReturn = pPath
                    End If
