...
#Region "Methods"
        ...
        Public Function GetItemsToDelete(ByVal isCleaner As Boolean, ByVal mMovie As Model.Movie) As List(Of IO.FileSystemInfo)
            Dim dPath As String = String.Empty
            Dim ItemsToDelete As New List(Of FileSystemInfo)
            Dim fScanner As New Scanner

            Try
                Dim MovieFile As New FileInfo(mMovie.MoviePath)
                Dim MovieDir As DirectoryInfo = MovieFile.Directory

                If Master.eSettings.VideoTSParent AndAlso isVideoTS(MovieDir.FullName) Then
                    dPath = String.Concat(Path.Combine(MovieDir.Parent.FullName, MovieDir.Parent.Name), ".ext")
                ElseIf Master.eSettings.VideoTSParent AndAlso isBDRip(MovieDir.FullName) Then
                    dPath = String.Concat(Path.Combine(MovieDir.Parent.Parent.FullName, MovieDir.Parent.Parent.Name), ".ext")
                Else
                    dPath = mMovie.MoviePath
                End If
                ...
                Else
                    ...
                    If Not isCleaner Then
                        ...
                        If Not String.IsNullOrEmpty(fPath) AndAlso File.Exists(fPath) Then
                            ...
                            Else
                                If Path.GetFileName(fPath).ToLower = "fanart.jpg" Then
                                    tPath = Path.Combine(Master.eSettings.BDPath, String.Concat(Path.GetFileNameWithoutExtension(mMovie.MoviePath), "-fanart.jpg"))
                                Else
                                    tPath = Path.Combine(Master.eSettings.BDPath, Path.GetFileName(fPath))
                                End If
                            End If
                    ...
                    If Not isCleaner AndAlso mMovie.UseFolder AndAlso Not Master.SourcesList.Contains(MovieDir.Parent.ToString) Then
                        If isVideoTS(MovieDir.FullName) Then
                            ItemsToDelete.Add(MovieDir.Parent)
                        ElseIf isBDRip(MovieDir.FullName) Then
                        ...
                        Else
                            ...
                            Else
                                ...
                                ItemsToDelete.Add(New IO.FileInfo(mMovie.MoviePath))
                            End If
                        End If
                    Else
                        ...
                        If Not isCleaner Then
                            ...
                            ioFi.Clear()
                            Try
                                If mMovie.UseFolder Then ioFi.AddRange(dirInfo.GetFiles(String.Concat(sOrName, "*.*")))
                            Catch
                            End Try

                            Try
                                ioFi.AddRange(dirInfo.GetFiles(String.Concat(Path.GetFileNameWithoutExtension(mMovie.MoviePath), ".*")))
                            Catch
                            End Try
